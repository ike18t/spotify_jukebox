...
describe SQLite3Adapter do
  ...
  context '[]' do
    it 'should pull the value for the key from the database' do
      ...
      SQLite3Adapter.new(test_db_name)['test_key'].should eq('test_value')
    end
  end

  context '[]=' do
    it 'should create a row in db for key if it does not exist' do
      connection.get_first_value('SELECT COUNT(*) FROM key_value_store WHERE key = ?', 'test_key').should eq(0)

      expected_value = 'bah'
      SQLite3Adapter.new(test_db_name)['test_key'] = expected_value
      connection.get_first_value('SELECT value FROM key_value_store WHERE key = ?', 'test_key').should eq(expected_value)
    end
    ...
    it 'should update the value for the given key' do
      ...
      connection.get_first_value('SELECT COUNT(*) FROM key_value_store WHERE key = ?', 'test_key').should eq(1)
