...
describe JukeboxPlayer do
  context 'get_tracks_for_user' do
    it 'should not return tracks that have been played recently' do
      ...
      spotify_wrapper.stubs(:get_tracks_for_collaborator).returns(tracks)
      ...
      jukebox_player.send(:get_random_track_for_user, 'playlist', 'user').should eql(nil)
    end
    ...
    it 'should return a track that has not been played recently' do
      ...
      spotify_wrapper = double(SpotifyWrapper)
      expect(spotify_wrapper).to receive(:get_tracks_for_collaborator).with('playlist', 'user').and_return(tracks)
      historian = double(TrackHistorian)
      expect(historian).to receive(:update_user_track_count).with('user', 3)
      tracks.each do |track|
      ...
      jukebox_player = JukeboxPlayer.new spotify_wrapper, nil, nil, historian
      jukebox_player.send(:get_random_track_for_user, 'playlist', 'user').should eql(expected)
    end
  end
end
